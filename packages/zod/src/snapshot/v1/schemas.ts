...
export const KeyEnumSchema = z.enum(['firstKey', 'secondKey', 'thirdKey']);
...
export const ComplexKeyMapBSchema = z
  .record(z.string())
  .superRefine((data, ctx) => {
    for (const key of Object.keys(data)) {
      const result = KeyEnumSchema.safeParse(key);
      if (result.success) continue;
...
export const AllMapsSchema = z.object({
  ...
  complexKeyMapB: ComplexKeyMapBSchema,
